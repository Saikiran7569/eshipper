...
export class PalletTypeComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.palletTypeService
      ...
      .subscribe((res: HttpResponse<IPalletType[]>) => this.paginatePalletTypes(res.body, res.headers));
  }
  ...
  transition() {
    this.router.navigate(['/pallet-type'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
  ...
  protected paginatePalletTypes(data: IPalletType[], headers: HttpHeaders) {
    this.links = this.parseLinks.parse(headers.get('link'));
    this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
    this.palletTypes = data;
  }
}
